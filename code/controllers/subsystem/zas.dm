...
SUBSYSTEM_DEF(zas)
	...
	var/list/datum/atmosphere/atmospheres = list()
...
/datum/controller/subsystem/zas/Initialize(timeofday, simulate = TRUE)
	...
	var/datum/gas_mixture/lavaland_air = atmospheres[/datum/atmosphere/planetary/lavaland]?.air_singleton
	if(lavaland_air)
		var/num_gases = length(lavaland_air.gas)
		to_chat(world, span_boldannounce("ZAS: Lavaland contains [num_gases] [num_gases > 1? "gases" : "gas"], with a pressure of [lavaland_air.returnPressure()] kpa."))
