...
/datum/color_matrix_editor/ui_interact(mob/user, datum/tgui/inferno/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "ColorMatrixEditor")
		ui.open()

/datum/color_matrix_editor/ui_act(action, list/params, datum/tgui/inferno/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
	switch(action)
