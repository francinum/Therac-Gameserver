...
#define STOP_PROCESSING(Processor, Datum) if(Datum.datum_flags & DF_ISPROCESSING) { Datum.datum_flags &= ~DF_ISPROCESSING;Processor.processing -= Datum;Processor.currentrun -= Datum }
